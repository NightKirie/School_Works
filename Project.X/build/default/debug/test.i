...
void RunMode()
{
  ...
  LATB = 0b00000001 | 0b00000010&0 | 0b00001000&0 | 0b00000100&0 | 0b00100000 | 0b00010000 ;
  LATB = 0b00000001 | 0b00000010&0 | 0b00001000&0 | 0b00000100&0 | 0b00100000&0 | 0b00010000&0 ;
  LATA = 0b00010000&0;
}
...
void LoadMode()
{
  ...
  LATB = 0b00000001 | 0b00000010 | 0b00001000 | 0b00000100 | 0b00100000 | 0b00010000 ;
  LATB = 0b00000001 | 0b00000010 | 0b00001000 | 0b00000100&0 | 0b00100000 | 0b00010000 ;
  LATB = 0b00000001 | 0b00000010 | 0b00001000 | 0b00000100 | 0b00100000 | 0b00010000 ;

  LATB = 0b00000001 | 0b00000010 | 0b00001000 | 0b00000100 | 0b00100000 | 0b00010000&0 ;
  TRISC = 0x00;
}
...
void write_SRAM_bytes(uint8_t value, uint8_t count)
{
  ...
  do {
    ...
    LATB = 0b00000001&0 | 0b00000010 | 0b00001000 | 0b00000100 | 0b00100000 | 0b00010000&0 ;
    LATB = 0b00000001 | 0b00000010 | 0b00001000 | 0b00000100 | 0b00100000 | 0b00010000&0 ;

    LATB = 0b00000001 | 0b00000010 | 0b00001000 | 0b00000100&0 | 0b00100000 | 0b00010000&0 ;
    LATB = 0b00000001 | 0b00000010 | 0b00001000 | 0b00000100 | 0b00100000 | 0b00010000&0 ;
  } while (--count);
}
...
{
  do {
    write_SRAM_bytes( vsync | 0b01000000 | rgb&0 , 16);
    write_SRAM_bytes( vsync | 0b01000000&0 | rgb&0 , 96);
    write_SRAM_bytes( vsync | 0b01000000 | rgb&0 , 48);
    write_SRAM_bytes( vsync | 0b01000000 | rgb , 200);
    write_SRAM_bytes( vsync | 0b01000000 | rgb , 200);
    write_SRAM_bytes( vsync | 0b01000000 | rgb , 240);
  } while (--count);
}
# 147 "test.c"
void GenerateFrame()
{
  GenerateLine( 0b10000000 , 0, 33);

  int16_t dc = ((int16_t)(((640./480.) * 0.125 / 640.) * (1<<12)));
  int16_t dd = ((int16_t)((0.125 / 480.) * (1<<12)));
  int16_t d = ((int16_t)((-.0625) * (1<<12)));
  int16_t row = 480;
  do {
    write_SRAM_bytes( 0b10000000 | 0b01000000 | 0 , 16);
    write_SRAM_bytes( 0b10000000 | 0b01000000&0 | 0 , 96);
    write_SRAM_bytes( 0b10000000 | 0b01000000 | 0 , 48);

    int16_t c = ((int16_t)((-1.5) * (1<<12)));
    int16_t col = 640;
    do {
      int16_t a = 0;
      int16_t b = 0;
      ...
      do {
        ...
        if (aa > ((int16_t)((4.) * (1<<12))) ||
            bb > ((int16_t)((4.) * (1<<12))) ||
            aa + bb > ((int16_t)((4.) * (1<<12)))){
          break;
        }
        b = (((int32_t)a * (int32_t)b) >> (12 -1)) + d;
        a = aa - bb + c;
      } while(--iter);
      ...
      green = ((iter & 0x0c) >> 2);
      blue = ((iter & 0x30) >> 4);
      write_SRAM_bytes( 0b10000000 | 0b01000000 | (((red) << 4) | ((green) << 2) | (blue)), 1);
  ...
  GenerateLine( 0b10000000 , 0, 10);
  GenerateLine( 0b10000000&0 , 0, 2);
  write_SRAM_bytes( 0b10000000 | 0b01000000 | 0, 2);
}
