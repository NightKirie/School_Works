...
void Mode2()
{
    ...
    PrintBuffer(strtok(GetString(), "$"));
    CCPR1 = atoi( strtok(GetString(), "$") );
    T3CONbits.TMR3ON = 1;
    while(PIR1bits.CCP1IF==0);
    if( RC2 == 1) PrintBuffer("True");
    else PrintBuffer("False");
    PIR1bits.CCP1IF=0;
    TMR3 = 0;
}
...
void main(void)
{
    ...
    while(1) {
        ...
        if(strstr(GetString(), "clear"))
            ClearBuffer();
        else if (strstr(GetString(), "mode1"))
            Mode1();
        else if (strstr(GetString(), "mode2"))
            Mode2();
        else if (strstr(GetString(), "mode3"))
            Mode3();
    }
    return;
